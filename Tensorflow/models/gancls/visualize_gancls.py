...
class GanClsVisualizer(object):
    def __init__(self, sess: tf.compat.v1.Session, model: GanCls, dataset: TextDataset, config):
        self.sess = sess
        self.model = model
    ...
    def visualize(self):
        z = tf.compat.v1.placeholder(tf.float32, [None, self.model.z_dim], name='z')
        cond = tf.compat.v1.placeholder(tf.float32, [None] + [self.model.embed_dim], name='cond')
        gen = self.model.generator(z, cond, is_training=False)

        saver = tf.compat.v1.train.Saver(tf.compat.v1.global_variables('g_net'))
